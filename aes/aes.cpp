...
void RoundInv(unsigned char * state, unsigned char * key) {
	AddRoundKey(state, key);
...
void AESDecrypt(unsigned char * encryptedMessage, unsigned char * expandedKey, unsigned char * decryptedMessage)
{
	...
	for (int i = 8; i >= 0; i--) {
		RoundInv(state, expandedKey + (16 * (i + 1)));
	}
...
int main() {
	...
	unsigned char * message = new unsigned char[16];
	...
	AESEncrypt(message, expandedKey, encryptedMessage);
	...
	cout << endl;
	unsigned char * decryptedMessage = new unsigned char[16];
